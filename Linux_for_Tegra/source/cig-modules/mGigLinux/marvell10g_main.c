...
static int mv3310_config_init(struct phy_device *phydev)
{
	...
	/* Force 10GBASER_RATE_MATCH mode for XFI MAC compatibility.
	 * Without rate matching, the PHY switches its serdes interface
	 * (e.g. to SGMII at 1G) which breaks fixed-10G MAC links.
	 */
	if (chip->set_mactype &&
	    mactype != MV_V2_33X0_PORT_CTRL_MACTYPE_10GBASER_RATE_MATCH) {
		phydev_info(phydev,
			    "Changing MACTYPE from %d to 10GBASER_RATE_MATCH\n",
			    mactype);
		err = chip->set_mactype(phydev,
					MV_V2_33X0_PORT_CTRL_MACTYPE_10GBASER_RATE_MATCH);
		if (err)
			return err;
		mactype = MV_V2_33X0_PORT_CTRL_MACTYPE_10GBASER_RATE_MATCH;
	...
	/* Configure LED0 and LED1 control registers: set bit0 to enable */
	err = phy_set_bits_mmd(phydev, MDIO_MMD_VEND2, 0xF020, BIT(0));
	if (err)
		return err;
	err = phy_set_bits_mmd(phydev, MDIO_MMD_VEND2, 0xF021, BIT(0));
	if (err)
		return err;
	return 0;
}
