				...
				u16 addr, u8 *val)
{
	...
	if(!err){
		*val = reg_val & 0xFF;
		dev_info(dev,"zuojisi:z_imx390_5200_9295:r addr(0x%04x)<-0x%02x\n",addr,*val);
	}
	if (err)
		dev_err(dev, "zuojisi:z_imx390_5200_9295: i2c read failed, addr=0x%x\n", addr);
				...
				u16 addr, u8 val)
{
	...
	if (err)
		dev_err(dev, "zuojisi:%s:z_imx390_5200_9295:i2c write failed, 0x%x = %x\n",
			__func__, addr, val);

	/* delay before next i2c command as required for SERDES link */
	usleep_range(100, 110);
	if(!err)
		dev_info(dev,"zuojisi:z_imx390_5200_9295:w addr(0x%04x)->0x%02x\n",addr,val);
	return err;
}
...
static int iic_write(struct i2c_client* client, u8 slaveaddr, u16 regaddr, u8 data)
{
	...
	usleep_range(100, 110);
	if(ret==1){ //send ok
		dev_info(&client->dev,"zuojisi:z_imx390_5200_9295:w slave=0x%02x addr(0x%04x)->0x%02x\n",slaveaddr,regaddr,data);
		return 0;
	}else{
		dev_err(&client->dev, "zuojisi:%s:z_imx390_5200_9295:i2c write failed, slave=0x%02x addr(0x%04x)->0x%02x\n",
...
static int iic_read(struct i2c_client* client, u8 slaveaddr, u16 regaddr, u8 *data)
{
	...
	if(ret==2){
		*data = buf[0];
		dev_info(&client->dev,"zuojisi:z_imx390_5200_9295:r slave=0x%02x addr(0x%04x)<-0x%02x\n",slaveaddr, regaddr,*data);
		return 0;
	}else{
		dev_err(&client->dev, "zuojisi: %s z_imx390_5200_9295: i2c read failed, slave=0x%02x, addr=0x%04x\n", __func__, slaveaddr, regaddr);
		return -1;
	}
}
...
static int z_imx390_5200_9295_set_gain(struct tegracam_device *tc_dev, s64 val)
{
...
static int z_imx390_5200_9295_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
    struct device *dev = s_data->dev;
...
static int z_imx390_5200_9295_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
...
static struct tegracam_ctrl_ops z_imx390_5200_9295_ctrl_ops = {
    ...
    .set_gain = z_imx390_5200_9295_set_gain,
    .set_exposure = z_imx390_5200_9295_set_exposure,
    .set_frame_rate = z_imx390_5200_9295_set_frame_rate,
...
{
	dev_info(tc_dev->dev,"zuojisi:z_imx390_5200_9295 set mode\n");
	return 0;
}
...
static int z_imx390_5200_9295_start_streaming(struct tegracam_device *tc_dev)
{
	...
	dev_info(dev,"zuojisi: z_imx390_5200_9295 start streaming - enter\n");

	err = z_max96724_start_streaming(priv->dser_dev, &priv->g_ctx);
	if (err)
		goto exit;

	/* Configure 9295 AFTER 96724 link reset to prevent registers being wiped */
...
#define MAX9295_GPIO_FRAME_TRIGGER 0x02D3
	...
	dev_info(dev,"zuojisi: z_imx390_5200_9295 start streaming - exit\n");
...
static int z_imx390_5200_9295_stop_streaming(struct tegracam_device *tc_dev)
{
	...
	dev_info(dev,"zuojisi:z_imx390_5200_9295 stop streaming\n");
...
static int z_imx390_5200_9295_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	...
	dev_info(dev, "zuojisi: probing v4l2 sensor @0x%x.\n",client->addr);
	...
	if (err || val != 0x91) {
		dev_err(dev, "zuojisi access z_imx390_5200_9295's 9295 failed\n");
...
*/
	...
	dev_info(&client->dev, "zuojisi:Detected z_imx390_5200_9295 sensor\n");
...
MODULE_DESCRIPTION("Media Controller driver for z_imx390_5200_9295");
